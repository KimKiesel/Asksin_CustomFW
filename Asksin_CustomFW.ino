...
//- relay pins (relay 1 = pin 12, no second physical relay)
const uint8_t pinRelay[] = {12};
#endif

#if defined(HM_LC_Sw2PBU_FM)
//- relay pins (relay 1 = pin 12, relay 2 = pin 13)
const uint8_t pinRelay[] = {12, 13};
#endif
...
void setup() {
	...
	bk[2].config(2,8,0,1000,5000,buttonState);
        ...
        #if defined(HM_LC_Sw2PBU_FM)
          // second physical relay
          pinMode(pinRelay[1], OUTPUT);
          rl[1].config(4,&relayState,&setInternalRelay,&hm,1,1);
        #endif
...
void relayState(uint8_t cnl, uint8_t curStat, uint8_t nxtStat) {
	...
	Serial << "c:" << cnl << " cS:" << curStat << " nS:" << nxtStat << '\n';	// some debug message
        #endif
        #if defined(LED_SHOW_RELAY_STATE)
...
void setInternalRelay(uint8_t cnl, uint8_t tValue) {
  //Switch physical relay 1 and 2 (if present) on channel 3 or 4
  //relay 1 pin is at pinRelay[0] and relay 2 pin is at pinRelay[1]
  digitalWrite(pinRelay[cnl - 3],tValue);
}
